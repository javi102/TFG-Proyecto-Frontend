...
@Component({
  selector: 'app-teemo-stats',
  templateUrl: './teemo-stats.component.html',
  styleUrls: ['./teemo-stats.component.css']
})
export class TeemoStatsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.championService.getStats2().subscribe(
      (response: any) => {
        ...
        if (response && response.data) {
          this.stats2 = response.data; // Guarda las estadísticas en la propiedad stats2
        } else {
