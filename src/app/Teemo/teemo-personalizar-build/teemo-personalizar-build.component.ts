...
@Component({
  selector: 'app-teemo-personalizar-build',
  templateUrl: './teemo-personalizar-build.component.html',
  styleUrls: ['./teemo-personalizar-build.component.css']
})
export class TeemoPersonalizarBuildComponent implements OnInit {
  build: any[] = [];
  builds: any;
  ...
  legendaryItems = [
    ...
    guardarBuild() {
      ...
      const buildData = { // Crear un objeto con los datos de la build
      ...
      this.championService.saveBuild(buildData).subscribe(
        (response: any) => {
    ...
    agregarItem(item: any) { // Agregar un ítem a la build
      console.log('Item a agregar:', item);  
      if (this.build.length < 12) { // Si la build tiene menos de 12 ítems podra seguir agregando hasta que alcance el limite de 12
        this.build.push(item); // Agregar el ítem a la build  gracias al metodo push 
      } else {
  ...
  eliminarUltimoItem() {
    this.build.pop(); // Eliminar el último ítem de la build gracias al metodo push 
  }
