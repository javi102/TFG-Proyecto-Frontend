...
@Component({
  selector: 'app-ahri-personalizar-build',
  templateUrl: './ahri-personalizar-build.component.html',
  styleUrls: ['./ahri-personalizar-build.component.css']
})
export class AhriPersonalizarBuildComponent implements OnInit {
  build: any[] = [];
  builds: any;
  championId: number = 2;  
  userId: number = 1;      
  ...
  legendaryItems = [
    ...
    agregarItem(item: any) {
      ...
      if (this.build.length < 12) { // Si la build tiene menos de 12 ítems
        this.build.push(item); // Agrega el ítem a la build gracias al método push 
      } else {
  ...
  eliminarUltimoItem() {
    this.build.pop(); // Elimina el último ítem de la build gracias al metodo pop 
  }
