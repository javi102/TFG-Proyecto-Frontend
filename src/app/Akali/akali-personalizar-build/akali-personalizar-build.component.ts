...
@Component({
  selector: 'app-akali-personalizar-build',
  templateUrl: './akali-personalizar-build.component.html',
  styleUrls: ['./akali-personalizar-build.component.css']
})
export class AkaliPersonalizarBuildComponent implements OnInit {
  build: any[] = [];
  builds: any;
  championId: number = 3;  
  userId: number = 0;      
  ...
  legendaryItems = [
    ...
    guardarBuild() {
      ...
      this.championService.saveBuild(buildData).subscribe( // Guardar la build en la base de datos
        (response: any) => {
    ...
    agregarItem(item: any) {
      console.log('Item a agregar:', item);  // Verifica qué contiene cada ítem antes de agregarlo
      if (this.build.length < 12) { // Verifica que no haya más de 12 ítems en la build
        this.build.push(item); // Agrega el ítem a la build
      } else {
  ...
  eliminarUltimoItem() {
    this.build.pop(); // Elimina el último ítem de la build 
  }
