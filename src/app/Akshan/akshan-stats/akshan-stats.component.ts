...
@Component({
  selector: 'app-akshan-stats',
  templateUrl: './akshan-stats.component.html',
  styleUrls: ['./akshan-stats.component.css']
})
export class AkshanStatsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.championService.getChampions().subscribe( 
      (response: any) => {
        ...
        if (response && response.data) {
          this.champions = response.data; // Asigna los campeones
        } else {
    ...
    this.championService.getStats2().subscribe(
      (response: any) => {
        ...
        if (response && response.data) {
          this.stats2 = response.data; // Asigna las estadísticas
        } else {
