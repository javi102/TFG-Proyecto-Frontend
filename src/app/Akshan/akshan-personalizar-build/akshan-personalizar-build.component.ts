...
@Component({
  selector: 'app-akshan-personalizar-build',
  templateUrl: './akshan-personalizar-build.component.html',
  styleUrls: ['./akshan-personalizar-build.component.css']
})
export class AkshanPersonalizarBuildComponent implements OnInit {
  build: any[] = [];
  builds: any;
  championId: number = 4;  
  userId: number = 0;      
  ...
  legendaryItems = [
    ...
    agregarItem(item: any) {
      ...
      if (this.build.length < 12) { // Te dejara guardar la build siempre y cuando no tenga mas de 12 items.
  ...
  eliminarUltimoItem() {
    this.build.pop(); // Eliminar el último ítem de la build gracias al metodo pop
  }
