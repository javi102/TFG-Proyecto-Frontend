...
@Component({
  selector: 'app-akshan-counter-matchup',
  templateUrl: './akshan-counter-matchup.component.html',
  styleUrls: ['./akshan-counter-matchup.component.css']
})
export class AkshanCounterMatchupComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.championService.getCounterMatchups().subscribe( // Obtiene los counter matchups
      (response: any) => {
        ...
        if (response && response.data) {
          this.counterMatchups = response.data.slice(33, 44);  
        } else {
    ...
    this.championService.getMatchups().subscribe( // Obtiene los matchups
      (response: any) => {
        ...
        this.matchups = response.data.slice(33, 44);  
      },
      (error) => {
    ...
    this.championService.getGoodMatchups().subscribe( // Obtiene los good matchups
      (response: any) => {
        ...
        this.goodMatchups = response.data.slice(33, 44);  
      },
      (error) => {
